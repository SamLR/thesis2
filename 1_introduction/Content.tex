...
\end{align}
where \( \mathcal{L} \) is the luminosity, \(\mathcal{L}_0\) is the luminosity at low specific ionisation density, \(k_B\) is Birks' constant which has to be measured for the scintillator. The main feature of Birks' Law is that rather than an linear response with deposited energy there is a quenching effect that means that at as more energy is deposited fewer photons are produced. For polyvinyltoluene (PVT, the base used for our scintillators), the response is approximately linear for energy depositions less than 0.5~MeV~mm\(^-1\)~\cite{birk_for_pvt} i.e.\ minimally ionising muons. The lower momenta muons (<\(56\)~MeV/c) slightly exceed this value~\cite{groom2001_mylar_ref} but not significantly.
